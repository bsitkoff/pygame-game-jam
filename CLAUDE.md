# This Assignment: Lesson 7 — Game Jam (Capstone)

Milestone assessment (not PRIMM). Students design and build a complete game from scratch. Three difficulty tiers: Mild (catcher game), Medium (original game with one new mechanic), Spicy (velocity-based motion with dx/dy). This is a no-AI assessment.

**Student files**: `game.py` (blank starter), `images/` (sprite library)
**Guide pages**: Planning → Coding → Playtest and Reflect
**Note**: Assessments haven't been created in Codio yet — guide pages have ASSESSMENT NEEDED placeholders. No solution files currently exist in `.guides/secure/`.
